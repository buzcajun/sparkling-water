...
object WriteConverterCtxUtils {
  ...
  type ConversionFunction[T] = (String, Array[Byte], UploadPlan) => SparkJob[T]
  type UploadPlan = Option[immutable.Map[Int, NodeDesc]]

  def create(uploadPlan: UploadPlan,
  ...
  def convert[T](hc: H2OContext, rdd : RDD[T], keyName: String, colNames: Array[String], vecTypes: Array[Byte],
    ...
    val (preparedRDD, uploadPlan) = if(hc.getConf.runsInExternalClusterMode){
      val res = ExternalWriteConverterCtx.scheduleUpload[T](rdd)
      (res._1, Some(res._2))
    }else{
      (rdd, None)
    }
